...
const CheckoutPickupPage: React.FC = () => {
  ...
  const pickupLocations: PickupLocation[] = [
    ...
    {
      id: 'midtown',
      name: 'Store Pickup - Midtown',
      address: '456 Center Avenue, Midtown City',
      readyTime: 'Usually ready in 1 hour',
      price: 'Free'
    },
    {
      id: 'uptown',
      name: 'Store Pickup - Uptown',
    ...
    {
      id: 'eastside',
      name: 'Store Pickup - Eastside',
      address: '1011 East Boulevard, Eastside City',
      readyTime: 'Usually ready in 4 hours',
      price: 'Free'
    }
  ];
  ...
  const handleContinueToPayment = () => {
    // Handle form submission and navigation to payment step
    console.log('Continue to payment:', formData);
    navigate('/checkout/payment');
  };

  const handleReturnToInformation = () => {
    // Navigate back to information step
    window.history.back();
  };
        ...
        <div className="flex-1 px-14 py-14 pl-[400px]">
          ...
          <div className="space-y-4 mb-10">
            ...
            <div className="space-y-0">
              {/* Ship Option */}
              <div className={`flex items-center gap-4 p-4 border-t border-l border-r ${formData.deliveryMethod === 'ship' ? 'border-black' : 'border-[#C8C8C8]'}`}>
              ...
              {/* Pickup Option */}
              <div className={`flex items-center gap-4 p-4 border ${formData.deliveryMethod === 'pickup' ? 'border-black' : 'border-[#C8C8C8]'}`}>
                <label className="flex items-center gap-2 cursor-pointer flex-1">
                  <div className="relative">
                    <input
                      type="radio"
          ...
          {/* Pick Up Address */}
          <div className="space-y-4 mb-10">
            <h2 className="text-[32px] font-medium leading-[1.1] tracking-[-4%]">Pick Up Address</h2>
            
            <div className="space-y-0">
              {pickupLocations.map((location, index) => (
                <div 
                  key={location.id} 
                  className={`border ${index === 0 ? 'border-t' : 'border-t-0'} ${formData.pickupLocation === location.id ? 'border-black' : 'border-[#C8C8C8]'} ${index === pickupLocations.length - 1 ? '' : 'border-b-0'}`}
                >
                  <div className="flex items-start gap-4 p-4">
                    <div className="flex-1">
                      <div className="space-y-1">
                        <label className="flex items-center gap-2 cursor-pointer">
                          <div className="relative">
                            <input
                              ...
                              onChange={() => handleInputChange('pickupLocation', location.id)}
                              className="sr-only"
                            />
                        ...
                        <div className="pl-7">
                          <p className="text-base font-medium text-[#969696]">{location.address}</p>
                        </div>
                      </div>
                    </div>
                    <div className="text-right space-y-1">
                      <div className="text-base font-medium text-black">{location.price}</div>
                      <div className="text-base font-medium text-[#969696]">{location.readyTime}</div>
                    </div>
                  </div>
                </div>
              ))}
        ...
        <div className="w-[600px] bg-[#F2F2F2] p-14 pr-80">
          <div className="space-y-8">
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-between items-center">
                <span className="text-base font-medium text-[#969696]">Pick Up</span>
                <span className="text-base font-medium text-black">Free</span>
              </div>
              <div className="flex justify-between items-center">
...
export default CheckoutPickupPage;
